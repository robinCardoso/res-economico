...
export class ClientePerfilAnalyticsService {
  ...
  async gerarRelatorioCliente(
    ...
    if (!nomeFantasia) {
      throw new Error(
        'Nome fantasia é obrigatório para gerar relatório de cliente',
      );
    }
    ...
    try {
      ...
      const [metricasArray, comportamentoArray, segmentacaoArray] =
        await Promise.all([
          this.metricasService
            .calcularMetricasFinanceiras(filtrosCliente)
            .catch((error) => {
              this.logger.error(
                `Erro ao calcular métricas financeiras: ${error.message}`,
              );
              return [];
            }),
          this.comportamentoService
            .analisarComportamentoCompra(filtrosCliente)
            .catch((error) => {
              this.logger.error(
                `Erro ao analisar comportamento de compra: ${error.message}`,
              );
              return [];
            }),
          this.segmentacaoService
            .segmentarClientes(filtrosCliente)
            .catch((error) => {
      ...
      const comportamento =
        comportamentoArray[0] || ({} as ComportamentoCompraCliente);
      ...
      // Gerar recomendações
      const recomendacoes = await this.alertasService
        .gerarRecomendacoes(segmentacao, metricas, comportamento)
        .catch((error) => {
      ...
      this.logger.error(
        `Erro ao gerar relatório do cliente ${nomeFantasia}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async gerarRelatoriosClientes(
    ...
    try {
      ...
      const comportamentoArray = await this.comportamentoService
        .analisarComportamentoCompra(filtros)
        .catch((error) => {
          this.logger.error(
            `Erro ao analisar comportamento de compra: ${error.message}`,
          );
      ...
      const [metricasArray, segmentacaoArray] = await Promise.all([
        this.metricasService
          .calcularMetricasFinanceiras(filtros)
          .catch((error) => {
            this.logger.error(
              `Erro ao calcular métricas financeiras: ${error.message}`,
            );
      ...
      const metricasMap = new Map(
        (metricasArray as MetricasFinanceirasCliente[]).map(
          (m) =>
            [this.gerarChaveCliente(m), m] as [
              string,
              MetricasFinanceirasCliente,
            ],
        ),
      ...
      const comportamentoMap = new Map(
        (comportamentoArray as ComportamentoCompraCliente[]).map(
          (c) =>
            [this.gerarChaveCliente(c), c] as [
              string,
              ComportamentoCompraCliente,
            ],
        ),
      ...
      const segmentacaoMap = new Map(
        (segmentacaoArray as SegmentacaoCliente[]).map(
          (s) => [this.gerarChaveCliente(s), s] as [string, SegmentacaoCliente],
        ),
      );
      ...
      for (const metricas of metricasArray) {
        if (!metricas || !metricas.nomeFantasia) {
          this.logger.warn(
            'Registro de métricas inválido encontrado, pulando...',
          );
          continue;
        }
        ...
        if (!comportamento || !segmentacao) {
          this.logger.warn(
            `Dados incompletos para cliente ${chave}, pulando...`,
          );
          continue;
        }
        ...
        try {
          ...
          const alertas = await this.gerarAlertasCliente(
            ...
            this.logger.error(
              `Erro ao gerar alertas para cliente ${metricas.nomeFantasia}: ${error.message}`,
            );
            return [];
          });
          ...
          const recomendacoes = await this.alertasService
            ...
            .catch((error) => {
              this.logger.error(
                `Erro ao gerar recomendações para cliente ${metricas.nomeFantasia}: ${error.message}`,
              );
              return [];
            });
          ...
          this.logger.error(
            `Erro ao processar cliente ${metricas.nomeFantasia}: ${innerError.message}`,
          );
      ...
      this.logger.error(
        `Erro ao gerar relatórios de clientes: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async gerarVisaoGeral(
    ...
    try {
      // First get the behavior data to reuse it in other calculations
      const comportamentosArray = await this.comportamentoService
        .analisarComportamentoCompra(filtros)
        .catch((error) => {
          this.logger.error(
            `Erro ao analisar comportamento de compra: ${error.message}`,
      ...
      const totalClientes = (metricasArray as MetricasFinanceirasCliente[])
        .length;

      // Clientes ativos (compraram nos últimos 90 dias)
      const hoje = new Date();
      const clientesAtivos = (
        metricasArray as MetricasFinanceirasCliente[]
      ).filter((m) => {
        ...
        try {
          const diasSemCompra = Math.floor(
            (hoje.getTime() - new Date(m.ultimaCompra).getTime()) /
              (1000 * 60 * 60 * 24),
          );
          return diasSemCompra <= 90;
        } catch (error) {
          this.logger.warn(
            `Data inválida para cliente ${m.nomeFantasia}: ${error.message}`,
          );
          return false;
        }
      }).length;
      ...
      const receitaTotal = (
        metricasArray as MetricasFinanceirasCliente[]
      ).reduce((sum: number, m: MetricasFinanceirasCliente) => {
      ...
      const ticketMedioGeral =
        metricasArray.length > 0
          ? (metricasArray as MetricasFinanceirasCliente[]).reduce(
              (sum: number, m: MetricasFinanceirasCliente) => {
              ...
              0,
            ) / metricasArray.length
          : 0;

      // LTV
      const lifetimeValueTotal = (
        metricasArray as MetricasFinanceirasCliente[]
      ).reduce((sum: number, m: MetricasFinanceirasCliente) => {
      ...
      const topClientes = [...metricasArray]
        ...
        .sort(
          (a: any, b: any) => (b?.receitaTotal || 0) - (a?.receitaTotal || 0),
        )
        .slice(0, 10)
        .map((m: any) => {
      ...
      const crescentes = tendencias.filter(
        (t: any) => t === 'crescente',
      ).length;
      const decrescentes = tendencias.filter(
        (t: any) => t === 'decrescente',
      ).length;
      ...
      const receitaPotencial = (
        segmentacaoArray as SegmentacaoCliente[]
      ).reduce((sum: number, s: SegmentacaoCliente) => {
      ...
      const clientesComOportunidade = (
        segmentacaoArray as SegmentacaoCliente[]
      ).filter((s) => s?.potencialCrescimento !== 'baixo').length;

      // Dados agregados para gráficos
      const receitaMensalAgregada =
        this.agruparReceitaMensalAgregada(metricasArray);
      const marcasMaisCompradas =
        this.agruparMarcasMaisCompradasFromComportamento(comportamentosArray);
      const sazonalidadeAgregada =
        this.agruparSazonalidadeAgregadaFromComportamento(comportamentosArray);
    ...
    try {
      ...
      if (metricas.ultimaCompra) {
        ...
        try {
          ...
          this.logger.warn(
            `Data de última compra inválida para cliente ${metricas.nomeFantasia}: ${error.message}`,
          );
        }
      ...
      try {
        const alertaQueda =
          await this.alertasService.gerarAlertasQuedaReceita(metricas);
        if (alertaQueda) {
          alertas.push(alertaQueda);
        }
      } catch (error) {
        this.logger.error(
          `Erro ao gerar alerta de queda de receita: ${error.message}`,
        );
      }
      ...
      try {
        ...
        this.logger.error(
          `Erro ao gerar alerta de oportunidade: ${error.message}`,
        );
      }
    } catch (error) {
      this.logger.error(`Erro ao gerar alertas do cliente: ${error.message}`);
   ...
   */
  private agruparReceitaMensalAgregada(
    metricasArray: MetricasFinanceirasCliente[],
  ) {
    ...
    for (const metricas of metricasArray) {
      if (
        metricas &&
        metricas.receitaMensal &&
        Array.isArray(metricas.receitaMensal)
      ) {
   ...
   */
  private agruparMarcasMaisCompradasFromComportamento(
    comportamentos: ComportamentoCompraCliente[],
  ) {
    ...
    for (const comportamento of comportamentos) {
      if (
        comportamento &&
        comportamento.marcasPrincipais &&
        Array.isArray(comportamento.marcasPrincipais)
      ) {
        for (const marca of comportamento.marcasPrincipais) {
          ...
          dados.quantidade +=
            typeof marca.quantidadeCompras === 'number'
              ? marca.quantidadeCompras
              : 0;
          dados.valor +=
            typeof marca.valorTotal === 'number' ? marca.valorTotal : 0;
        }
      }
    }
  ...
  private async agruparMarcasMaisCompradas(filtros: FiltrosPerfilClienteDto) {
    ...
    const comportamentos =
      await this.comportamentoService.analisarComportamentoCompra(filtros);
   ...
   */
  private agruparSazonalidadeAgregadaFromComportamento(
    comportamentos: ComportamentoCompraCliente[],
  ) {
    ...
    for (const comportamento of comportamentos) {
      if (
        comportamento &&
        comportamento.padraoSazonal?.receitaPorMes &&
        Array.isArray(comportamento.padraoSazonal.receitaPorMes)
      ) {
